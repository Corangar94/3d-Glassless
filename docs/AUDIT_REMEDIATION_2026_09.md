...
| F08 exact-SHA assurance | Required checks gate release publication for the actual tag commit. Windows checks cover both supported Python versions; native and package workflows have no path-filter blind spots. Master protection requires the six GitHub-Actions check contexts and an up-to-date PR, including for admins; force pushes and branch deletion are disabled. Independent approval count remains zero for single-maintainer use. |
...
| F11 dependency reproducibility | Isolated CI virtual environments with exact installed-closure validation and full transitive hashed Windows locks for Python 3.11 and 3.12; no editable build isolation or dependency re-resolution. Use a single OpenCV contrib distribution. Existing release manifest retains pinned native/model/toolchain inventory in addition to Python SBOM. |
