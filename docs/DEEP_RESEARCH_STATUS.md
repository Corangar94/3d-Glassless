...
| Add repeatable depth validation fixtures | `tracker.depth_fixtures` loads manifest-backed `.npy` depth fixtures, the repo includes a `synthetic_static_smoke` baseline, and both evaluation suite/support bundles accept `--depth-fixture`. |
