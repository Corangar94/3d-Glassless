...
rewriting or `ShowWindow` interception is used. Build/deployment failure is fatal;
all three native files must match `Glassless3DOverlay.build.json`. The executable
is generated by bootstrap and is no longer tracked in Git.

CI installs reviewed hashed Windows Python 3.11/3.12 locks via
`python -m scripts.locked_environment`. Native tests are run with
`python -m scripts.run_native_tests`; these include compiled production HLSL and
synthetic DirectML integration, not webcam/monitor hardware acceptance.

See `docs/AUDIT_REMEDIATION_2026_09.md` for the finding-to-fix map.
...
**IMPORTANT**: A locked destination now fails deployment. Stop the overlay and rerun bootstrap; verify the SHA-256 build sidecar rather than relying on timestamps.
...
### 1. Native deployment must be verified

CMake copies the executable and runtime DLLs with checked commands and writes
`Glassless3DOverlay.build.json` only after successful deployment. Locked files
fail the build; stop the overlay and rerun bootstrap. Do not manually mix files
