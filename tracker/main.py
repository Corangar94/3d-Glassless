...
def _configure_camera(cap: cv2.VideoCapture, width: int = 0, height: int = 0, fps: float = 0.0) -> None:
    requested = (
...
def _log_camera_opened(index: int, backend_name: str, cap: cv2.VideoCapture) -> None:
...
def _load_face_tracker_class(backend: str):
    ...
    if backend_id not in {"auto", "mediapipe", "cv2"}:
        raise SystemExit("ERROR: tracking backend must be one of: auto, mediapipe, cv2")
...
def _validated_live_calibration(settings: OverlaySettings | None) -> tuple[float | None, float | None]:
    if settings is None:
        return None, None
...
def _measurement_noise(settings: OverlaySettings | None) -> float:
    ...
    return _DEFAULT_SMOOTHING_R if parsed is None or parsed <= 0.0 else max(parsed, 1e-6)


def _apply_live_calibration(tracker: FaceTrackerLike, settings: OverlaySettings | None, previous: tuple[float | None, float | None] | None) -> tuple[float | None, float | None] | None:
...
def _validated_pose(position: HeadPosition | None) -> HeadPosition | None:
    if position is None:
        return None
    x, y, z = _finite_float(position.x_cm), _finite_float(position.y_cm), _finite_float(position.z_cm)
    ...
    return HeadPosition(
        ...
        capture_timestamp_ms=int(getattr(position, "capture_timestamp_ms", 0) or monotonic_ms()) & 0xFFFF_FFFF,
    )


def _limit_pose_step(raw: tuple[float, float, float], prev: tuple[float, float, float] | None, max_xy_step_cm: float = 10.0, max_z_step_cm: float = 12.0) -> tuple[float, float, float]:
    if prev is None:
        return raw
...
def _tilt_filtered_pose(pose: FilteredPose, tilt_deg: float) -> FilteredPose:
    if abs(tilt_deg) < 1e-6:
        return pose
    x, y, z = _apply_camera_tilt(pose.x_cm, pose.y_cm, pose.z_cm, tilt_deg)
    vx, vy, vz = _apply_camera_tilt(pose.vx_cm_s, pose.vy_cm_s, pose.vz_cm_s, tilt_deg)
    return FilteredPose(
        x_cm=x, y_cm=y, z_cm=z,
        vx_cm_s=vx, vy_cm_s=vy, vz_cm_s=vz,
        yaw_deg=pose.yaw_deg, pitch_deg=pose.pitch_deg - tilt_deg, roll_deg=pose.roll_deg,
...
class TrackingLoop:
    def __init__(self, tracker: FaceTrackerLike, writer: PoseWriterLike, smoother: HeadSmoother | AdaptivePoseFilter, hold_ms: int = 500, stop_event: Optional[threading.Event] = None, camera_tilt_deg: float = 0.0, config_path: Optional[str] = None, camera_quality_monitor: CameraQualityMonitor | None = None, lock_camera_controls: bool = False) -> None:
        ...
        self._last_output_pose = FilteredPose(x_cm=0.0, y_cm=0.0, z_cm=60.0)
    ...
    def _process_frame(self, frame: object, capture_timestamp_ms: int) -> HeadPosition | None:
        try:
            return self._tracker.process_frame(frame, capture_timestamp_ms=capture_timestamp_ms)
        except TypeError:
    ...
    def _update_filter(self, pose: HeadPosition) -> FilteredPose:
        ...
        dt_s = measurement_s - self._last_measurement_s if self._last_measurement_s is not None else None
        self._last_measurement_s = measurement_s
        xyz = self._smoother.update(pose.x_cm, pose.y_cm, pose.z_cm) if dt_s is None else self._smoother.update(pose.x_cm, pose.y_cm, pose.z_cm, dt_seconds=dt_s)
        return FilteredPose(x_cm=xyz[0], y_cm=xyz[1], z_cm=xyz[2], yaw_deg=pose.yaw_deg, pitch_deg=pose.pitch_deg, roll_deg=pose.roll_deg, confidence=pose.confidence, capture_timestamp_ms=pose.capture_timestamp_ms, publish_timestamp_ms=monotonic_ms())
    ...
    def run(self, camera_index: int = 0, camera_width: int = 0, camera_height: int = 0, camera_fps: float = 0.0, max_frames: Optional[int] = None) -> None:
        ...
        try:
            while not self._should_stop():
                ...
                measured = _validated_pose(self._process_frame(frame, capture_timestamp_ms))

                if measured is not None:
                    self._last_face_ms = time.monotonic() * 1000.0
                    limited = _limit_pose_step(measured.xyz, self._last_raw_pos)
                    self._last_raw_pos = limited
                    measured = HeadPosition(x_cm=limited[0], y_cm=limited[1], z_cm=limited[2], yaw_deg=measured.yaw_deg, pitch_deg=measured.pitch_deg, roll_deg=measured.roll_deg, confidence=measured.confidence, capture_timestamp_ms=measured.capture_timestamp_ms)
                    output, status = self._update_filter(measured), "tracking"
                else:
                ...
                self._last_output_pose, self._last_smoothed = output, output.xyz
    ...
    def _should_stop(self) -> bool:
        return self._stop_event.is_set() if self._stop_event is not None else False
...
def main() -> None:
    ...
    parser.add_argument("--tracking-backend", choices=("auto", "mediapipe", "cv2"), default=None, help="face tracking backend; auto prefers MediaPipe and falls back to OpenCV")
    ...
    with (
        ...
        class _MultiWriter:
            def write(self, x: float, y: float, z: float) -> None:
                ft_writer.write(x=x, y=y, z=z); g3d_writer.write(x=x, y=y, z=z)
