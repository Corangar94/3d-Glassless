...
velocity, orientation, confidence, and camera/publish timestamps so the native
overlay does not estimate and filter motion a second time.
"""
...
class PoseStateWriter:
    def __init__(self, name: str = POSE_V2_NAME) -> None:
        self._name = name
        self._handle: int | None = _k32.CreateFileMappingW(_INVALID_HANDLE, None, _PAGE_READWRITE, 0, POSE_V2_SIZE, name)
        if not self._handle:
            raise ctypes.WinError(ctypes.get_last_error())
        self._view: int | None = _k32.MapViewOfFile(self._handle, _FILE_MAP_ALL_ACCESS, 0, 0, POSE_V2_SIZE)
        if not self._view:
            error = ctypes.get_last_error()
            _k32.CloseHandle(self._handle)
            self._handle = None
            raise ctypes.WinError(error)
        self._seq_handle: int | None = _k32.CreateFileMappingW(_INVALID_HANDLE, None, _PAGE_READWRITE, 0, POSE_V2_SEQ_SIZE, f"{name}_Seq")
        self._seq_view: int | None = None
        if self._seq_handle:
            self._seq_view = _k32.MapViewOfFile(self._seq_handle, _FILE_MAP_ALL_ACCESS, 0, 0, POSE_V2_SEQ_SIZE)
        if self._seq_view:
            ctypes.c_uint32.from_address(self._seq_view).value = 0
        self.write(FilteredPose(x_cm=0.0, y_cm=0.0, z_cm=60.0, publish_timestamp_ms=monotonic_ms()), valid=False)

    def write(self, pose: FilteredPose, *, valid: bool = True) -> None:
        if self._view is None:
            raise RuntimeError("write() called after close()")
        values = (pose.x_cm, pose.y_cm, pose.z_cm, pose.vx_cm_s, pose.vy_cm_s, pose.vz_cm_s, pose.yaw_deg, pose.pitch_deg, pose.roll_deg, pose.confidence)
        ...
        if any(abs(value) > 1e-4 for value in (pose.yaw_deg, pose.pitch_deg, pose.roll_deg)):
            flags |= PoseFlags.ORIENTATION_VALID
        data = struct.pack(POSE_V2_FORMAT, POSE_V2_MAGIC, POSE_V2_VERSION, *[float(value) for value in values], int(pose.capture_timestamp_ms) & 0xFFFF_FFFF, int(pose.publish_timestamp_ms or monotonic_ms()) & 0xFFFF_FFFF, int(flags), 0)
        sequence = ctypes.c_uint32.from_address(self._seq_view) if self._seq_view else None
        if sequence is not None:
            sequence.value = (sequence.value + 1) & 0xFFFF_FFFF
...
class PoseStateReader:
    ...
    def _try_attach(self) -> None:
        ...
        if not self._handle:
            self._handle = None
            return
        self._view = _k32.MapViewOfFile(self._handle, _FILE_MAP_READ, 0, 0, POSE_V2_SIZE)
        if not self._view:
            _k32.CloseHandle(self._handle)
            self._handle = None
            return
        self._seq_handle = _k32.OpenFileMappingW(_FILE_MAP_READ, False, f"{self._name}_Seq")
        if self._seq_handle:
            self._seq_view = _k32.MapViewOfFile(self._seq_handle, _FILE_MAP_READ, 0, 0, POSE_V2_SEQ_SIZE)
    ...
    def read(self) -> PosePacketV2 | None:
        ...
        if not all(math.isfinite(float(value)) for value in values):
            return None
        return PosePacketV2(x_cm=values[0], y_cm=values[1], z_cm=values[2], vx_cm_s=values[3], vy_cm_s=values[4], vz_cm_s=values[5], yaw_deg=values[6], pitch_deg=values[7], roll_deg=values[8], confidence=values[9], capture_timestamp_ms=unpacked[12], publish_timestamp_ms=unpacked[13], flags=PoseFlags(unpacked[14]))
