...
from tracker.face_tracker import FaceTracker, estimate_xy_cm, estimate_z_cm
...
def test_estimate_xy_right_of_centre():
    """Nose right of centre → positive X offset (mirrored camera)."""
    x, _ = estimate_xy_cm(
        nose_x_norm=0.3,
        nose_y_norm=0.5,
        z_cm=60.0,
...
def test_face_tracker_init():
    """FaceTracker constructs with the verified bootstrap model when present."""
    ...
    if not os.path.isfile(model_path):
        pytest.skip(
            "verified face model is installed by the bootstrap/native-build gate"
        )
    with FaceTracker(
        real_ipd_cm=6.3,
...
def test_estimate_xy_left_of_image_gives_positive_x():
    """Nose left of image centre → positive X (user's right, mirrored)."""
    x, _ = estimate_xy_cm(
        nose_x_norm=0.3,
        nose_y_norm=0.5,
        z_cm=60.0,
